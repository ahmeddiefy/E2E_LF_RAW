...
def model(input_tensor):
    with tf.device("/gpu:0"):
        ...
        print(tensor.shape)
        tensor = tf.concat( [tf.expand_dims(input_tensor[:,:,:,0],axis=3), tensor[:,:,:,1:7], tf.expand_dims(input_tensor[:,:,:,1],axis=3), tensor[:,:,:,8:56], tf.expand_dims(input_tensor[:,:,:,2],axis=3), tensor[:,:,:,57:63], tf.expand_dims(input_tensor[:,:,:,3],axis=3)], axis=3)
        print(tensor.shape)
        
        upscaled_tensor = tf.depth_to_space(tensor, block_size=8)
        
        print(tensor.shape)
        tensor = spatial_augmentation_network(tensor, "Spatial_first")
        print(tensor.shape)
