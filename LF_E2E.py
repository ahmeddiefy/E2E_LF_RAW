...
def get_image_batch(train_list,offset,batch_size):
	...
	gt_list.resize([BATCH_SIZE, OUT_IMG_SIZE[0], OUT_IMG_SIZE[1], 1])
...
if __name__ == '__main__':
    ...
    if not USE_QUEUE_LOADING:
        ...
        train_gt  		= tf.placeholder(tf.float32, shape=(BATCH_SIZE, OUT_IMG_SIZE[0], OUT_IMG_SIZE[1], 1))
        ...
        train_gt_single  	= tf.placeholder(tf.float32, shape=(OUT_IMG_SIZE[0], OUT_IMG_SIZE[1], 1))
        q = tf.FIFOQueue(10000, [tf.float32, tf.float32], [[IMG_SIZE[0], IMG_SIZE[1], IMG_SIZE[2]], [OUT_IMG_SIZE[0], OUT_IMG_SIZE[1], 1]])
    ...
    with tf.Session(config=config) as sess:
        ...
        def load_and_enqueue(coord, file_list, enqueue_op, train_input_single, train_gt_single, idx=0, num_thread=1):
            ...
            try:
                while not coord.should_stop():
                    ...
                    gt_img		= scipy.io.loadmat(file_list[i][0])['patch'].reshape([OUT_IMG_SIZE[0], OUT_IMG_SIZE[1], 1])
        ...
        if USE_QUEUE_LOADING:
            ...
            for epoch in range(0, MAX_EPOCH):
                ...
                saver.save(sess, "./checkpoints/LF_E2E_adam_epoch_%03d.ckpt" % epoch ,global_step=global_step)
        else:
            for epoch in range(0, MAX_EPOCH):
                ...
                saver.save(sess, "./checkpoints/LF_E2E_const_clip_0.01_epoch_%03d.ckpt" % epoch ,global_step=global_step)
