...
def model(input_tensor):
    with tf.device("/gpu:0"):
        ...
        print(tensor.shape)
        tensor = tf.concat( [tensor[:,:,:,0:18], tf.expand_dims(input_tensor[:,:,:,0],axis=3), tensor[:,:,:,19:21], tf.expand_dims(input_tensor[:,:,:,1],axis=3), tensor[:,:,:,22:42], tf.expand_dims(input_tensor[:,:,:,2],axis=3), tensor[:,:,:,43:45], tf.expand_dims(input_tensor[:,:,:,3],axis=3), tensor[:,:,:,46:64],], axis=3)
        print(tensor.shape)
